...
{
    public partial class SF003 : DanhMucBase
    {
       #region ---- Variables ----

        DanhMucDanTocBLL _bussDanToc = null;
        ...
        protected override void GetNewData()
        {
            ...
            this.txtMaDanToc.Focus();
            DM_DanToc item = new DM_DanToc();
        ...
        protected override void Deletedata()
        {
            ...
            DM_DanToc item = brscGrdData.Current as DM_DanToc;
            int a = -1;

            if (item != null)
            {
                if (!AllowDeleteData("DM_DanToc"))
                {
                ...
                if (UICommon.ShowMsgConfirm("MSG006") == DialogResult.Yes)
                {
                    ...
                    if (item.Id != 0)
                    {
                        _bussDanToc.DeleteData(item.Id);
        ...
        protected override void SaveData()
        {
            ...
            if (Validator() == true)
            {
                ...
                if (brscGrdData.Count > 0)
                {
                    ...
                    _lastUpdate = 1;
                    List<DM_DanToc> list = (List<DM_DanToc>)brscGrdData.DataSource;
                    _bussDanToc.UpdateDataList(list);
                    UICommon.StopUpdate();
                    UICommon.ShowSplashPanelUpdateMsg();
        ...
        private void InitForm()
        {
            ...
            // Get data
            _bussDanToc = new DanhMucDanTocBLL();
            _listError = new List<int>();
            this.brscGrdData.DataSource = _bussDanToc.GetAll();
        ...
        private void Enable(bool pvalue)
        {
            ...
            txtMaDanToc.Enabled = pvalue;
            txtTenDanToc.Enabled = pvalue;
        }
        ...
        private void AddDataBinding()
        {
            ...
            txtMaDanToc.DataBindings.Add("Text", brscGrdData, "MaDanToc", true, DataSourceUpdateMode.OnPropertyChanged);
            txtTenDanToc.DataBindings.Add("Text", brscGrdData, "TenDanToc", true, DataSourceUpdateMode.OnPropertyChanged);
        ...
        private bool Validator()
        {
            ...
            List<DM_DanToc> listData = (List<DM_DanToc>)brscGrdData.DataSource;

            foreach (DM_DanToc item in listData)
            {
                // Get The position of the Item
                int a = listData.IndexOf(item);

                if (string.IsNullOrEmpty(item.MaDanToc))// MaChuyen Nganh nott null
                {
                ...
                if (string.IsNullOrEmpty(item.TenDanToc)) // Ten chuyen nganh not null
                {
                    UICommon.ShowMsgInfo("MSG005", lblTenChuyenNganh.Text);
                    this.txtTenDanToc.Focus();
                    _listError.Add(a);
                    brscGrdData.Position = a;
                    _listError.Add(a);
                    return false;
                ...
                List<DM_DanToc> listIndex = listData.Where(p => p.MaDanToc == item.MaDanToc).Select(p => p).ToList();

                // Check IsExited MaChuyenNganh in Grid
                if (listIndex.Count() > 1)
                {
                    foreach (DM_DanToc index in listIndex)
                    {
