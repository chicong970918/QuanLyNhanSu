...
{
    public partial class SF201 : GridBaseForm
    {
        #region ---- Variables ----
        ...
        protected override void GetNewData()
        {
            ...
            {
                UICommon.ShowMsgInfo("MSG017", lblQuy.Text,(((CacheData.Context.GetSystemDate().Month - 1) / 3) + 1).ToString());
                return;
            }
            ...
            item.MaUngVien = UICommon.GetString("MSG024");
            item.GioiTinh = true;
            item.TuyenThang = true;
            SF111 frm = new SF111();
        ...
        protected override void Deletedata()
        {
            ...
            TD_UngVien item = brscGrdData.Current as TD_UngVien;
            if (item != null)
            {
                if (_bussUngVien.CheckedUsing(item.Id))
                {
                ...
                else
                {
                    if (UICommon.ShowMsgConfirm("MSG006") == DialogResult.Yes)// Xac nhan
                    {
                        ...
                        if (item.Id != 0)
                        {
                            _bussUngVien.DeleteData(item.Id);
        ...
        {
            List<TD_UngVien> list = _bussUngVien.GetDataByPhongBanQuyNamTuyenThang(((DM_PhongBan)this.cboPhongBan.SelectedItem).Id, CommonUtil.IsInt(this.cboQuy.Text), CommonUtil.IsInt(txtNam.Text));
